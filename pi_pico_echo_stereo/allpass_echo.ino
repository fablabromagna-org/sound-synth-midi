...
// Sandro Grassia
// "allpass_echo.ino"
// Implementa una versione dello "Schroeder all-pass-filter"
...
uint16_t out_value_L = 0;
uint16_t out_value_R = 0;
uint16_t a_L = 0;
uint16_t a_R = 0;
...
bool repeating_timer_callback(struct repeating_timer *t)
{
    ...
    i2s_L.write(out_value_L);
    i2s_L.write(out_value_L);
    // aggiorna I2S_R ch.0 e ch.1
    i2s_R.write(out_value_R);
    i2s_R.write(out_value_R);
    ...
    // uscita
    a_L = in_value_L - K * D_fifo_L[D_read_sample_L];
    out_value_L = K * a_L + D_fifo_L[D_read_sample_L];

    // scrittura D_fifo
    D_fifo_L[D_write_sample_L] = F * a_L;
    ...
    // uscita
    a_R = in_value_R - K * D_fifo_R[D_read_sample_R];
    out_value_R = K * a_R + D_fifo_R[D_read_sample_R];

    // scrittura D_fifo
    D_fifo_R[D_write_sample_R] = F * a_R;

    // tempo di elaborazione: deve essere < 25!!. La differenza 25-T è tempo disponibile per ulteriori upgrade
    T1 = micros();
    T = T1 - T0;
    return true;
}
